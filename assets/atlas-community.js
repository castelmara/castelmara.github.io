(function () {
  ...
  async function openRosterEditor() {
    ...
    try {
      ...
      form.innerHTML = '<label>персонаж<input name="character_id" type="text" list="atlasRosterCharacterList" placeholder="найти персонажа из каталога" autocomplete="off" required><datalist id="atlasRosterCharacterList">'+rosterCharacterOptions()+'</datalist></label><label>раздел<select name="kind"><option value="athlete">команда</option><option value="cheer">группа поддержки</option></select></label><label>команда<select name="team_id">'+teams.map(t => '<option value="'+esc(t.id)+'">'+esc(t.name)+'</option>').join('')+'</select></label><label>позиция<select name="position"></select></label><label>роль<select name="captaincy"><option value="none">участник / участница</option><option value="captain">капитан</option><option value="vice-captain">заместитель капитана</option><option value="reserve captain">резервный капитан</option></select></label><label><input type="checkbox" name="visible" checked> показывать в составе</label><div class="atlas-community-form-actions"><button type="button" data-community-roster-new>новая запись</button><button type="submit">сохранить</button></div><small>найди и выбери существующего персонажа из каталога. при смене капитана сначала снимите эту роль с предыдущего.</small>';
  ...
  async function saveRoster(form) {
    ...
    if (!row.character_id) throw new Error('Выберите существующего персонажа из каталога.');
