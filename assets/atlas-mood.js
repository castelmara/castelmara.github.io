(function(){
  ...
  function put(text){const root=document.getElementById('atlasMoodResult');if(!root)return;root.replaceChildren();const prefix=document.createElement('span');prefix.textContent='hoy eres…';const value=document.createElement('strong');value.textContent=text;root.append(prefix,value)}
  async function render(){
    ...
    if(!user){put('inicia sesión para descubrirlo');if(explain)explain.textContent='ATLAS elige tu estado de ánimo una vez al día. Habrá uno nuevo después de la medianoche, hora peninsular.';return}
    if(cached?.key===key){put(cached.phrase);if(explain)explain.textContent=cached.explanation;return}if(pending?.key===key)return;
    const request={key};pending=request;put('buscando tu estado de ánimo…');
    try{if(!window.ATLAS_SUPABASE)throw new Error('Sin conexión');let timer,response;try{response=await Promise.race([window.ATLAS_SUPABASE.rpc('atlas_today_mood'),new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('El servidor no responde')),20000)})])}finally{clearTimeout(timer)}const {data,error}=response;if(error)throw error;if(!data?.length)throw new Error('Respuesta vacía');if(token!==generation||uid()!==user||key!==uid()+'|'+dateKey())return;const original=String(data[0].phrase||'').trim(),translated=moods[original.toLocaleLowerCase('ru-RU')]||'sorpresa de Castelmara';let description='ATLAS te ha asignado este título por hoy. Recíbelo con humor: mañana llegará uno nuevo.';if(original.toLocaleLowerCase('ru-RU')==='сладкая булочка')description='Por favor, guarda el cuchillo. Hoy toca cuidarte, darte de comer y espolvorearte algún que otro cumplido.';cached={key,phrase:translated,explanation:description+' El estado cambia a medianoche, hora peninsular.'};put(cached.phrase);if(explain)explain.textContent=cached.explanation}
    catch(_err){if(token===generation&&uid()===user){put('no se pudo cargar; pulsa para intentarlo de nuevo');if(explain)explain.textContent='Comprueba tu conexión. Al volver a cargar se conservará el estado elegido para hoy.'}}
    finally{if(pending===request)pending=null}
  }
  document.addEventListener('click',e=>{const toggle=e.target.closest('#atlasMoodExplainToggle');if(toggle){const open=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!open));toggle.textContent=open?'¿qué significa esto?':'entendido';const explanation=document.getElementById('atlasMoodExplain');if(explanation)explanation.hidden=open;return}if(e.target.closest('#atlasMoodResult')){if(!uid())document.getElementById('atlasAccountButton')?.click();else render()}if(e.target.closest('.atlas-nav-item,[data-open-page]'))setTimeout(render,80)});
